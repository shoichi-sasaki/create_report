...
def create_report(date_today):
    df = pd.read_csv(const.RAW_DATA_PATH + const.FILE_NAMES['agent'], names = ('エージェント', '開始時刻', '終了時間', '待機時間(秒)', '取得数' , '入電数', '放棄呼数', '通話時間(秒)'),header=None, skiprows=2)
    df = df.fillna(0)
    agent_timezones = {}
    contents = {
        '取得数', 
    ...
    for index, row in df.iterrows():
        ...
        if row['エージェント'] not in agent_timezones.keys():
            agent_timezones[row['エージェント']] = {} 
            for content in contents:
                agent_timezones[row['エージェント']][content] = [0]*24
        for content in contents:
            agent_timezones[row['エージェント']][content][start_hour] += row[content]

    def restruct(acc, cur):
        acc['エージェント'].append(cur)
        acc['エージェント'].extend(['']*23)
        acc['日付'].append(date_today.strftime('%Y/%m/%d'))
        acc['日付'].extend(['']*23)
        acc['時間'].extend([str(i)+':00' for i in range(24)])
        for item in init_arr.keys():
            if item == 'エージェント' or item == '日付' or item == '時間':
                continue
